...
function refreshTicker(tickerdata) {
  ...
  tickerdata.map((item) => {
    ...
    const tblBTCbody = document
      .getElementById("btcticker")
...
function tapeTicker(tickerdata) {
  if (usdArr.length > 1) {
    ...
    ticker.innerHTML = `Bitcoin (BTC): <b>${roundNumber(
      ...
      tickerdata[0].target
    )} <i class="seperator"></i> <b>${roundNumber(
      tickerdata[1].price
    )}</b> GBP ${getDirection(
      tickerdata[1].target
    )} <i class="seperator"></i> <b>${roundNumber(
      ...
      tickerdata[3].price
    )}</b> JPY ${getDirection(tickerdata[3].target)}<br>
...
function getDirection(currency) {
  switch (currency) {
    case "USD":
      return usdArr[usdArr.length - 2] === usdArr[usdArr.length - 1]
        ? '<i class="nochange"></i>'
        : usdArr[usdArr.length - 2] > usdArr[usdArr.length - 1]
        ? '<i class="down"></i>'
        : '<i class="up"></i>';
    case "GBP":
      return gbpArr[gbpArr.length - 2] === gbpArr[gbpArr.length - 1]
        ? '<i class="nochange"></i>'
        : gbpArr[gbpArr.length - 2] > gbpArr[gbpArr.length - 1]
        ? '<i class="down"></i>'
        : '<i class="up"></i>';
    case "EUR":
      return eurArr[eurArr.length - 2] === eurArr[eurArr.length - 1]
        ? '<i class="nochange"></i>'
        : eurArr[eurArr.length - 2] > eurArr[eurArr.length - 1]
        ? '<i class="down"></i>'
        : '<i class="up"></i>';
    case "JPY":
      return jpyArr[jpyArr.length - 2] === jpyArr[jpyArr.length - 1]
        ? '<i class="nochange"></i>'
        : jpyArr[jpyArr.length - 2] > jpyArr[jpyArr.length - 1]
        ? '<i class="down"></i>'
        : '<i class="up"></i>';
  }
}
